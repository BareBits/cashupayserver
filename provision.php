 ...
 *   - Single use: the first successful collection stamps
 *     provision_consumed_at in config and every later attempt gets 410.
 *     Consumption is recorded in the database rather than by rewriting
 *     user_config.php — this endpoint never modifies deployment files.
...
if (Config::get('provision_consumed_at')) {
    cashupay_status(410);
    echo json_encode(['error' => 'Provisioning credentials were already collected']);
    exit;
}

// The wizard's first-run flow creates exactly one store; on the off chance an
// operator added more before the installer collected, the original (first
// created) store is the one the orchestrator provisioned for.
...
// Seed the cron key if the wizard never rendered the cron screen (provisioned
// installs skip it via CASHUPAY_EXTERNAL_CRON) — same lazy seeding the cron
// screen and admin Settings use.
