...
def _poll_invoice_until(shared_configured: ConfiguredPayserver, invoice_id: str, status: str, timeout_s: float = 30) -> dict:
...
def test_invoice_payment_settles_end_to_end(
    ...
    gc.create_webhook(shared_configured.store_id, webhook_url, secret="wh-secret-e2e")

    # Create an invoice in sats — bypasses the fiat rate fetch.
    invoice = gc.create_invoice(shared_configured.store_id, amount=INVOICE_AMOUNT_SAT, currency="sat")
    ...
    settled = _poll_invoice_until(shared_configured, invoice_id, "Settled", timeout_s=30)
