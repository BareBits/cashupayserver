...
def _enable_auto_melt(
    ...
    Returns the response (includes lnurl_supports_verify from the LUD-21
    probe that the handler runs synchronously)."""
...
def test_lnurl_lud21_missing_falls_back_to_mint(
    ...
    save_result = _enable_auto_melt(configured)
    assert save_result.get("success") is True, save_result
    assert save_result.get("lnurl_supports_verify") == 0, (
        "host without verify URL should report unsupported; "
        f"got {save_result}"
    )

    # Stores table mirrors the response.
    store_row = _read_store_row(configured.handle, configured.store_id)
    assert store_row.get("lnurl_supports_verify") == 0, store_row
