...
def _poll_until(shared_configured: ConfiguredPayserver, invoice_id: str, status: str, timeout_s: float = 30) -> dict:
...
def test_xpub_validation_admin_endpoint(shared_configured: ConfiguredPayserver, onchain: OnchainContext) -> None:
    """Admin's validate_onchain_xpub action returns valid + a 3-address preview."""
    r = shared_configured.admin.s.post(
...
def test_xpub_rejects_mainnet_on_regtest_store(shared_configured: ConfiguredPayserver) -> None:
    mainnet_xpub = (
...
def test_address_derivation_parity_with_bitcoin_cli(
    ...
    start_index = _wire_onchain(shared_configured, onchain)
    addrs_from_cashupay = []
    for _ in range(3):
        ...
        shared_configured.greenfield.mark_invoice_status(shared_configured.store_id, inv["id"], "Invalid")
...
def test_zero_conf_payment_settles(
    ...
    _wire_onchain(shared_configured, onchain, min_confs=0)
    inv = shared_configured.greenfield.create_invoice(
        shared_configured.store_id, amount=str(INVOICE_AMOUNT_SAT), currency="sat"
    )
    addr = inv["checkout"]["paymentMethods"]["BTC-OnChain"]["destination"]
...
def test_confirmed_payment_settles_after_block(
    ...
    _wire_onchain(shared_configured, onchain, min_confs=1)
    inv = shared_configured.greenfield.create_invoice(
        shared_configured.store_id, amount=str(INVOICE_AMOUNT_SAT), currency="sat"
    )
    addr = inv["checkout"]["paymentMethods"]["BTC-OnChain"]["destination"]
    ...
    shared_configured.handle.trigger_cron()
    pending = shared_configured.greenfield.get_invoice(shared_configured.store_id, inv["id"])
...
def test_pending_page_links_mempool_tx(
    ...
    while time.monotonic() < deadline:
        shared_configured.handle.trigger_cron()
        r = requests.get(
            f"{shared_configured.handle.url}/payment.php",
    ...
    html = requests.get(
        f"{shared_configured.handle.url}/payment.php", params={"id": inv["id"]}, timeout=15
    ).text
    ...
    onchain.confirm(1)
    _poll_until(shared_configured, inv["id"], "Settled", timeout_s=20)
    r = requests.get(
        f"{shared_configured.handle.url}/payment.php",
        params={"id": inv["id"], "json": "1"},
        timeout=15,
...
def test_address_uniqueness_across_invoices(
    ...
    _wire_onchain(shared_configured, onchain)
    seen = set()
    for _ in range(3):
        inv = shared_configured.greenfield.create_invoice(
            shared_configured.store_id, amount="1000", currency="sat"
        )
        addr = inv["checkout"]["paymentMethods"]["BTC-OnChain"]["destination"]
