...
def test_admin_user_exists_after_setup(shared_configured: ConfiguredPayserver) -> None:
...
def test_login_with_wrong_password_fails(configured: ConfiguredPayserver) -> None:
    # Failed-login tests stay on the function-scoped `configured` fixture:
    # every 401 feeds the global lockout counter, which must not leak into
    # a shared server's other tests.
    fresh = requests.Session()
    r = fresh.post(
...
def test_username_lookup_is_case_insensitive(shared_configured: ConfiguredPayserver) -> None:
...
def test_session_cookie_rotates_on_login(shared_configured: ConfiguredPayserver) -> None:
    ...
    s.get(f"{shared_configured.handle.url}/admin", timeout=15)
...
def test_authenticated_requests_use_user_id_session(shared_configured: ConfiguredPayserver) -> None:
    ...
    r = s.get(f"{shared_configured.handle.url}/admin?api=dashboard", timeout=15)
