...
// stores.onchain_source_order column) AND the older migration artifacts.
assert_true(has_col('stores', 'onchain_source_order'), 'fresh install has the current marker column');
assert_true(has_col('stores', 'ln_rail_order'), 'fresh install has the lightning rail-order column');
assert_true(has_col('invoices', 'onchain_last_polled_at'), 'fresh install has the previous marker column');
...
Database::getInstance()->exec("ALTER TABLE stores DROP COLUMN onchain_source_order");
Database::getInstance()->exec("ALTER TABLE stores DROP COLUMN ln_rail_order");
...
assert_false(has_col('stores', 'onchain_source_order'), 'pre-merge: marker column dropped');
...
assert_true(has_col('stores', 'onchain_source_order'), 'getInstance() self-heals the marker column');
assert_true(has_col('stores', 'ln_rail_order'), 'getInstance() self-heals the lightning rail-order column');
assert_true(has_col('invoices', 'onchain_last_polled_at'), 'getInstance() self-heals the previous marker column');
...
assert_true(has_col('stores', 'onchain_source_order'), 're-run leaves the marker column in place');
