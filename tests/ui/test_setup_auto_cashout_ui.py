"""Setup wizard step 9: choose where auto-cashouts go.
...
Run with: pytest tests/ui/test_setup_auto_cashout_ui.py -v -s
"""
...
def _walk_to_auto_cashout_step(page, payserver, mint, store_name="AW Store") -> None:
    """Drive a fresh setup wizard to step 9 (auto-cashout).

    Under the new step order, auto-cashout comes immediately after
...
def test_auto_cashout_lightning_persists_address(
    payserver: PayserverHandle, mint: MintHandle, page
) -> None:
    _walk_to_auto_cashout_step(page, payserver, mint, "LN Store")

    # Lightning is the pre-selected radio. Fill the address + submit.
    page.fill("#lightning_address", LIGHTNING_ADDRESS)
    page.locator("#auto-cashout-form button[type=submit]").click()
...
def test_auto_cashout_onchain_makes_onchain_step_required(
    payserver: PayserverHandle, mint: MintHandle, page
) -> None:
    _walk_to_auto_cashout_step(page, payserver, mint, "Onchain Store")

    # Pick the on-chain radio + submit.
    page.locator('input[name="auto_cashout_mode"][value="onchain"]').check()
    page.locator("#auto-cashout-form button[type=submit]").click()
    ...
    assert skip_buttons == 0, "Skip should be hidden when on-chain auto-cashout was picked"
...
def test_auto_cashout_skip_disables_auto_melt(
    payserver: PayserverHandle, mint: MintHandle, page
) -> None:
    _walk_to_auto_cashout_step(page, payserver, mint, "Skip Store")

    # The skip button is in a sibling form below the warning. Click the
    # one whose form has auto_cashout_action=skip to disambiguate from
...
def test_auto_cashout_step_visible_in_add_store_mode(
    payserver: PayserverHandle, mint: MintHandle, page
) -> None:
    ...
    _walk_to_auto_cashout_step(page, payserver, mint, "Initial")
    page.locator(
        'form:has(input[name="auto_cashout_action"][value="skip"]) '
        'button:has-text("Skip for now")'
    ).click()
    ...
    # (auto-cashout) before any mint/seed configuration.
    page.wait_for_selector("#auto-cashout-form", state="visible")
