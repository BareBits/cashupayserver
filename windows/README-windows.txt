CashuPayServer — Windows desktop edition
========================================

Run CashuPayServer as a local point-of-sale on any Windows 10/11 PC or
...
1. Extract this zip somewhere you have write access — your Desktop,
   Documents, or C:\CashuPayServer are all fine.
   Do NOT use C:\Program Files (the app needs to write its database).
2. Double-click CashuPayServer.bat.
...
A console window titled "CashuPayServer" stays open while the app runs.
Close that window to stop the app. A second, minimized window
("CashuPayServer background tasks") keeps invoice polling and webhook
delivery ticking; it closes itself shortly after you stop the server.


Day-to-day use
--------------
- Start: double-click CashuPayServer.bat (double-clicking while it is
  already running just reopens the browser).
- Stop: close the CashuPayServer console window.
...
CashuPayServer folder while the app is stopped is a complete backup.
...
2. Extract the new cashupayserver-windows-*.zip next to the old folder.
